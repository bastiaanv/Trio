...
/// Coalesces and serializes Nocturne upload runs per pipeline.
///
/// Runs of the same pipeline never overlap: concurrent runs would each fetch the
/// rows still marked `isUploadedToNS == false` and POST them twice, and Nocturne
...
actor NocturneUploadSerializer {
    /// Uploads everything still pending for a pipeline.
    private let uploadOperation: @Sendable(NocturneUploadPipeline) async -> Void

    /// Called when `run(_:)` is invoked from inside the same pipeline's upload
    /// operation. Asserts by default; tests inject a recorder.
    private let onReentrantRun: @Sendable(NocturneUploadPipeline) -> Void

    /// Pipeline whose upload operation the current task is executing, if any.
    @TaskLocal private static var activePipeline: NocturneUploadPipeline?

    /// Run currently in flight per pipeline.
    private var current: [NocturneUploadPipeline: Task<Void, Never>] = [:]
    /// Follow-up run queued behind the current one, at most one per pipeline.
    private var queued: [NocturneUploadPipeline: Task<Void, Never>] = [:]

    init(
        uploadOperation: @escaping @Sendable(NocturneUploadPipeline) async -> Void,
        onReentrantRun: @escaping @Sendable(NocturneUploadPipeline) -> Void = { pipeline in
    ...
    func request(_ pipeline: NocturneUploadPipeline) {
    ...
    func run(_ pipeline: NocturneUploadPipeline) async {
    ...
    private func scheduleRun(_ pipeline: NocturneUploadPipeline) -> Task<Void, Never> {
    ...
    private func promoteQueuedRun(_ pipeline: NocturneUploadPipeline) {
        current[pipeline] = queued[pipeline]
        queued[pipeline] = nil
    }

    private func finishCurrentRun(_ pipeline: NocturneUploadPipeline) {
